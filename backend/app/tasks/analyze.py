...
from app.services.eodhd_client import (
    normalize_eodhd_to_internal_format,
...
def analyze_company_task(
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS", meta={"progress": 10, "status": "Loading company data..."}
        )
        ...
        if should_hydrate_country(company.get("country")) and company.get("ticker"):
            yahoo_country = resolve_country_from_yahoo_asset_profile(
                company.get("ticker")
            )
            if yahoo_country:
        ...
        if should_hydrate_country(company.get("country")):
            hydrated_country = hydrate_country_with_eodhd(
                company.get("ticker"), company.get("exchange")
            )
            if hydrated_country:
                company["country"] = (
                    normalize_country(hydrated_country) or hydrated_country
                )
        ...
        if company.get("country") != original_country:
            try:
                ...
                print(
                    f"Analyze task: failed to persist hydrated country for {company_id}: {exc}"
                )
        company_name = company["name"]

        self.update_state(
            state="PROGRESS",
            meta={"progress": 20, "status": "Loading financial statements..."},
        )
            ...
            if "totalRevenue" in str(first_statement):
                ...
                for stmt in financial_statements:
                    ...
                    if "income_statement" in statements:
                        eodhd_structure["income_statement"]["quarterly"][period] = (
                            statements["income_statement"]
                        )
                    if "balance_sheet" in statements:
                        eodhd_structure["balance_sheet"]["quarterly"][period] = (
                            statements["balance_sheet"]
                        )
                    if "cash_flow" in statements:
                        eodhd_structure["cash_flow"]["quarterly"][period] = statements[
                            "cash_flow"
                        ]

                # Normalize to our internal format
                merged_financial_data = normalize_eodhd_to_internal_format(
                    eodhd_structure
                )

        self.update_state(
            state="PROGRESS", meta={"progress": 30, "status": "Calculating ratios..."}
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"progress": 40, "status": "Calculating health score..."},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"progress": 50, "status": "Generating AI summary..."},
        )
        ...
        if filing_ids:
            filing_response = (
                supabase.table("filings").select("*").eq("id", filing_ids[0]).execute()
            )
        ...
        summary_data = ai_client.generate_company_summary(
        ...
        if hasattr(ai_client, "normalize_summary_sections"):
            summary_data = ai_client.normalize_summary_sections(summary_data)
...
"""
        ...
        self.update_state(
            state="PROGRESS",
            meta={"progress": 70, "status": "Generating persona views..."},
        )
        ...
        self.update_state(
            state="PROGRESS", meta={"progress": 90, "status": "Saving results..."}
        )
                ...
                for line_item, values in stmt_data[statement_type].items():
                    if line_item not in merged[statement_type]:
                        ...
                        if isinstance(values, dict) and isinstance(
                            merged[statement_type][line_item], dict
                        ):
